...
function countProfit(shoppers) { return [['Sepatu Stacattu', 1500000, 10], ['Baju Zoro', 500000, 2], ['Sweater Uniklooh', 175000, 1]].map(([product, price, stock]) => ({ product, shoppers: shoppers.filter(s => s.product === product && (stock -= s.amount) >= 0).map(s => s.name), leftOver: stock, totalProfit: price * (stock = stock < 0 ? stock + shoppers.filter(s => s.product === product).reduce((a, b) => a + b.amount, 0) : stock, 10 - stock) })); }
